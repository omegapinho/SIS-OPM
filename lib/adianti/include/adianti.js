...
function __adianti_append_page(page)
{
    ...
    $.post(uri, params_json, function(data)
    {
       ...
       __adianti_error('Error', 'Erro na Rotina append_page' + textStatus);
    });
}
...
function __adianti_append_page2(page)
{
    ...
    $.post(uri, params_json, function(data)
    {
       ...
       __adianti_error('Error','Erro na Rotina append_page 2 ' + textStatus);
    });
}
    ...
    if (typeof page !== 'undefined')
    {
        ...
        if (url.indexOf('&static=1') > 0)
        {
            $.get(url, function(data)
            {
               ...
               __adianti_error('Error', 'Erro na Rotina load_Page, trecho static ' + textStatus);
            });
        }
        else
        {
            $.get(url, function(data)
            {
               ...
               __adianti_error('Error', 'Erro na Rotina load_page, trecho não static ' + textStatus);
            });
        }
    }
}
...
function __adianti_post_data(form, action)
{
    ...
    console.log('DATA GERAL ' + data);
    if (url.indexOf('&static=1') > 0 || (action.substring(0,3) == 'xhr'))
    {
        $.post(url, data,
            function(result) {
                __adianti_parse_html(result);
                __adianti_unblock_ui();
                console.log('Resulta do XHR ' + result);
                ...
                __adianti_unblock_ui();
               __adianti_error('Error', 'Erro na Rotina post_data, trecho XHR' +  + textStatus);
            });
    }
    else
    {
        $.post(url, data,
            function(result) {
                console.log('DATA para HTML ' + data);
                console.log('URL para HTML ' + url);
                __adianti_load_html(result, Adianti.onAfterPost);
                __adianti_unblock_ui();
            }).fail(function(jqxhr, textStatus, exception) {
                __adianti_unblock_ui();
               __adianti_error('Error', 'Erro na Rotina post_data, trecho HTML ' + + textStatus);
            });
    }
}
...
function __adianti_post_data2(form, url)
{
    ...
    $.post(url, data,
        function(result)
        {
            ...
            __adianti_unblock_ui();
               __adianti_error('Error', 'Erro na Rotina post_data2' + textStatus);
            });
}
...
function __adianti_ajax_exec(action, callback, automatic_output)
{
    ...
    $.ajax({
      url: uri
         ...
         __adianti_error('Error', 'Erro na Rotina ajax_exec ' + textStatus);
      });
}
...
function __adianti_get_page(action, callback, static_call)
{
    ...
    $.ajax({
      url: uri
         ...
         __adianti_error('Error', 'Erro na Rotina Get_Page' + textStatus);
      });
}
...
function __adianti_post_lookup(form, action, field, callback) {
    ...
    $.ajax({
      ...
      data: formdata
         ...
         __adianti_error('Error', 'Erro na Rotina LookUP ' + textStatus);
      });
}
...
function __adianti_parse_html(data, callback)
{
    ...
    try {
        ...
        if (e instanceof Error) {
            alert(e.message + ': ' + tmp);//Foi descomentado
