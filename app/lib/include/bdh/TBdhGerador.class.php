 ...
 *------------------------------------------------------------------------------*/
     ...
     *-------------------------------------------------------*/
    	/*$sql = "DELETE FROM bdhoras.bdh_historico_trabalho WHERE bdh_historico_trabalho_rg=$PM_RG ".
    			"AND bdh_historico_trabalho_dt_hora_inicio >= '$datas 00:00:00' AND ".
    			"bdh_historico_trabalho_dt_hora_inicio < '$dt_fim_corrigido' and ".
    			"(bdh_historico_trabalho_status='P' or bdh_historico_trabalho_status='T')  ;";	
    	$volta = $con->Executa ( $sql, 'bdhoras', 'bdh_historico_trabalho' );*/
        try 
        { 
            TTransaction::open('sicad'); 
 ...
 * ==================================================================================================*/
        		$sql = "SELECT datafim FROM bdhoras.historicotrabalho WHERE " .
        		                    "datafim BETWEEN '" . $dt_hora_inicio . " 00:00:00' AND '" . 
                                     $dt_hora_inicio . " 23:59:59' AND rgmilitar='" . $rgmilitar . "';";
        		$ex = self::pesquisa_afasta ($sql,'SELECT');
 ...
 * ==================================================================================================*/
        		if ($ex) 
        		{
        			$sql = "UPDATE bdhoras.historicotrabalho SET status='T', afastamentos_id='" . 
        			        ...
        			        $dt_hora_inicio . " 23:59:59' AND rgmilitar='" . $rgmilitar. "';";
        			//var_dump( $sql);exit;
        			$result = self::pesquisa_afasta ($sql,'UPDATE');
        			if (!$result)
        			{
                       $falha = true;
                    }
 ...
 * ==================================================================================================*/
                    ...
                    $result = self::pesquisa_afasta($sql,'INSERT');
        			if (!$result)
        			{
                       $falha = true;
                    }
        	...
        	$sql = "SELECT datafim FROM bdhoras.historicotrabalho ". 
                       "WHERE datafim BETWEEN '" . $dt_hora_inicio . " 00:00:00' AND '" . 
                       $dt_hora_termino . " 23:59:59' AND rgmilitar='" .  $rgmilitar . "';" ;
        	$dt_fim_ultimo_lancamento = self::pesquisa_afasta ($sql,'SELECT');
            //var_dump($dt_fim_ultimo_lancamento);
 ...
 * =================================================================================================*/
        	...
        	if ($dt_fim_ultimo != "") 
        	{
        		...
        		{
        			$slq = "UPDATE bdhoras.historicotrabalho SET status='T', afastamentos_id='" . $afasta_id . "', ". 
                    ...
                    $dt_fim_ultimo_lancamento = self::pesquisa_afasta ($sql,'UPDATE');
        			if (!$dt_fim_ultimo_lancamento)
        			{
                       $falha = true;
                    }
        		}
 ...
 *----------------------------------------------------------------------------------------*/
    public function close_escala ($opm,$mes,$ano,$inativo = 'S',$toda_OPM = 'S')
    {
        ...
        if ($dt_fim) 
        {
            ...
            else    //$toda_OPM == 'S' - Fecha toda OPM independente do militar estar mais nela ou que teve informação por ele
            {
                $aux = "";
                if ($inativo == 'S')
                {
                    $aux = "AND historicotrabalho.rgmilitar IN (SELECT DISTINCT rgmilitar "
                            . "FROM efetivo.servidor"
                            . " WHERE unidadeid=$opm"
                            . $status
                            . " order by rgmilitar)";
                }
                //2018-03-24 -> Correção para fechar pela OPM informante também 
                $sql = "UPDATE bdhoras.historicotrabalho "
                        . "SET status='T' "
                        . "WHERE (opm_id='" . $opm . "' OR opm_id_info='" . $opm  
                        . "') AND (status='P')"
                        . " AND datainicio"
                        . " BETWEEN '$dt_inicio' AND '$dt_fim' ". $aux .";";
                $ret = self::grava_fechamento($sql,"UPDATE");
                if (!$ret)
                {
                    $retorno = false;
 ...
 *
 *====================================================================================*/
    public function main_fechar ($unidades,$lista_opm, $action = "CLOSE",
                                 $mes=null,$ano=null,$fecha_inativo = 'S',$toda_OPM = 'S')
    {
/*--------------------------------------------------------------------
 * Nota: Rotina de Seleção de Ação(action). Podendo ser:
