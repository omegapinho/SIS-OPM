<?php
/**
 * orgaosorigemList Listing
 * @author  <your name here>
 */
class orgaosorigemList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_orgaosorigem');
        $this->form->class = 'tform'; // change CSS class
        
        $this->form->style = 'display: table;width:100%'; // change style
        $this->form->setFormTitle('Listagem dos Órgãos de Origem');
        ...
        $this->form->setData( TSession::getValue('orgaosorigem_filter_data') );
        
        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'),  new TAction(array('orgaosorigemForm', 'onEdit')), 'bs:plus-sign green');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('orgaosorigemForm', 'onEdit'));
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new orgaosorigem($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('orgaosorigemList_filter_nome',   NULL);
        TSession::setValue('orgaosorigemList_filter_sigla',   NULL);
        TSession::setValue('orgaosorigemList_filter_oculto',   NULL);

        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'like', "%{$data->nome}%"); // create the filter
            TSession::setValue('orgaosorigemList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->sigla) AND ($data->sigla)) {
            $filter = new TFilter('sigla', 'like', "%{$data->sigla}%"); // create the filter
            TSession::setValue('orgaosorigemList_filter_sigla',   $filter); // stores the filter in the session
        }


        if (isset($data->oculto) AND ($data->oculto)) {
            $filter = new TFilter('oculto', '=', "$data->oculto"); // create the filter
            TSession::setValue('orgaosorigemList_filter_oculto',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('orgaosorigem_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for orgaosorigem
            ...
            if (TSession::getValue('orgaosorigemList_filter_nome')) {
                $criteria->add(TSession::getValue('orgaosorigemList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('orgaosorigemList_filter_sigla')) {
                $criteria->add(TSession::getValue('orgaosorigemList_filter_sigla')); // add the session filter
            }


            if (TSession::getValue('orgaosorigemList_filter_oculto')) {
                $criteria->add(TSession::getValue('orgaosorigemList_filter_oculto')); // add the session filter
            }
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('sisacad'); // open a transaction with database
            $object = new orgaosorigem($key, FALSE); // instantiates the Active Record
