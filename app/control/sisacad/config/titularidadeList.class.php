<?php
/**
 * titularidadeList Listing
 * @author  <your name here>
 */
class titularidadeList extends TPage
{
    private $form; // form
 ...
 *------------------------------------------------------------------------------*/
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_titularidade');
        $this->form->class = 'tform'; // change CSS class
        
        $this->form->style = 'display: table;width:100%'; // change style
        $this->form->setFormTitle('Listagem de titularidade');
        ...
        $this->form->addQuickField('Nível', $nivel,  80 );
        $this->form->addQuickField('Oculto?', $oculto,  120 );

        
        // keep the form filled during navigation with session data
        $this->form->setData( TSession::getValue('titularidade_filter_data') );
        
        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'),  new TAction(array('titularidadeForm', 'onEdit')), 'bs:plus-sign green');
        ...
        $column_nivel = new TDataGridColumn('nivel', 'Nível', 'right');
        ...
        $this->datagrid->addColumn($column_nivel);
        $this->datagrid->addColumn($column_oculto);


        // creates the datagrid column actions
        $order_nivel = new TAction(array($this, 'onReload'));
        $order_nivel->setParameter('order', 'nivel');
        $column_nivel->setAction($order_nivel);
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('titularidadeForm', 'onEdit'));
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new titularidade($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('titularidadeList_filter_nivel',   NULL);
        TSession::setValue('titularidadeList_filter_nome',   NULL);
        TSession::setValue('titularidadeList_filter_oculto',   NULL);

        if (isset($data->nivel) AND ($data->nivel)) {
            $filter = new TFilter('nivel', '>=', "$data->nivel"); // create the filter
            TSession::setValue('titularidadeList_filter_nivel',   $filter); // stores the filter in the session
        }


        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'like', "%{$data->nome}%"); // create the filter
            TSession::setValue('titularidadeList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->oculto) AND ($data->oculto)) {
            $filter = new TFilter('oculto', '=', "$data->oculto"); // create the filter
            TSession::setValue('titularidadeList_filter_oculto',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('titularidade_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for titularidade
            $repository = new TRepository('titularidade');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('titularidadeList_filter_nivel')) {
                $criteria->add(TSession::getValue('titularidadeList_filter_nivel')); // add the session filter
            }


            if (TSession::getValue('titularidadeList_filter_nome')) {
                $criteria->add(TSession::getValue('titularidadeList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('titularidadeList_filter_oculto')) {
                $criteria->add(TSession::getValue('titularidadeList_filter_oculto')); // add the session filter
            }
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('sisacad'); // open a transaction with database
            $object = new titularidade($key, FALSE); // instantiates the Active Record
