<?php
/**
 * nivel_pagamentoList Listing
 * @author  <your name here>
 */
class nivel_pagamentoList extends TPage
{
    private $form; // form
 ...
 *------------------------------------------------------------------------------*/
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_nivel_pagamento');
        $this->form->class = 'tform'; // change CSS class
        
        $this->form->style = 'display: table;width:100%'; // change style
        $this->form->setFormTitle('Listagem dos níveis de pagamento');
        ...
        $this->form->setData( TSession::getValue('nivel_pagamento_filter_data') );
        
        // add the search form actions
        $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $this->form->addQuickAction(_t('New'),  new TAction(array('nivel_pagamentoForm', 'onEdit')), 'bs:plus-sign green');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('nivel_pagamentoForm', 'onEdit'));
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new nivel_pagamento($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('nivel_pagamentoList_filter_nome',   NULL);
        TSession::setValue('nivel_pagamentoList_filter_oculto',   NULL);

        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'like', "%{$data->nome}%"); // create the filter
            TSession::setValue('nivel_pagamentoList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->oculto) AND ($data->oculto)) {
            $filter = new TFilter('oculto', '=', "$data->oculto"); // create the filter
            TSession::setValue('nivel_pagamentoList_filter_oculto',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('nivel_pagamento_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for nivel_pagamento
            $repository = new TRepository('nivel_pagamento');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('nivel_pagamentoList_filter_nome')) {
                $criteria->add(TSession::getValue('nivel_pagamentoList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('nivel_pagamentoList_filter_oculto')) {
                $criteria->add(TSession::getValue('nivel_pagamentoList_filter_oculto')); // add the session filter
            }
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('sisacad'); // open a transaction with database
            $object = new nivel_pagamento($key, FALSE); // instantiates the Active Record
