 ...
 *------------------------------------------------------------------------------*/
        ...
        $this->form->setFormTitle('Listagem das disciplinas');
        ...
        $this->form->addQuickAction(_t('New'),  new TAction(array('disciplinaForm', 'onEdit')), 'bs:plus-sign green');
        ...
        $action_edit = new TDataGridAction(array($this, 'onDisciplina'));
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('disciplinaList_filter_nome',   NULL);
        TSession::setValue('disciplinaList_filter_sigla',   NULL);
        TSession::setValue('disciplinaList_filter_oculto',   NULL);

        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'like', "%{$data->nome}%"); // create the filter
            TSession::setValue('disciplinaList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->sigla) AND ($data->sigla)) {
            $filter = new TFilter('sigla', 'like', "%{$data->sigla}%"); // create the filter
            TSession::setValue('disciplinaList_filter_sigla',   $filter); // stores the filter in the session
        }


        if (isset($data->oculto) AND ($data->oculto)) {
            $filter = new TFilter('oculto', '=', "$data->oculto"); // create the filter
            TSession::setValue('disciplinaList_filter_oculto',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('disciplinaList_filter_nome')) {
                $criteria->add(TSession::getValue('disciplinaList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('disciplinaList_filter_sigla')) {
                $criteria->add(TSession::getValue('disciplinaList_filter_sigla')); // add the session filter
            }


            if (TSession::getValue('disciplinaList_filter_oculto')) {
                $criteria->add(TSession::getValue('disciplinaList_filter_oculto')); // add the session filter
            }
...
/*------------------------------------------------------------------------------
 *    Direciona para Gerir Disciplinas e professores
 *------------------------------------------------------------------------------*/
 public function onDisciplina ($param = null)
 {
     if ($param)
     {
          TSession::setValue('defineDisciplinaProfessorForm_return','config');
          TApplication::loadPage('defineDisciplinaProfessorForm','onEdit',array('key'=>$param['key']));
          //var_dump($data);
     }
     $data = $this->form->getData();
     $this->form->setData($data);
