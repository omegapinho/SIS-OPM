 ...
 */
class documentos_turmaList extends TPage
{
    private $form; // form
 ...
 *------------------------------------------------------------------------------*/
        ...
        $this->form->setFormTitle('Gestão de documentos de turma');
        ...
        $this->form->addQuickAction('Registro de Aula',  new TAction(array($this, 'onComprovante')), 'bs:check green');
        ...
        $column_data_aula = new TDataGridColumn('data_aula', 'Data da Aula', 'center');
        ...
        $order_data_aula = new TAction(array($this, 'onReload'));
        $order_data_aula->setParameter('order', 'data_aula');
        $column_data_aula->setAction($order_data_aula);
        ...
        $column_data_aula->setTransformer( function($value, $object, $row) {
            $date = new DateTime($value);
            return $date->format('d/m/Y');
        });
        
        // create EDIT action
        $action_edit = new TDataGridAction(array('documentos_turmaForm', 'onEdit'));
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $repository = new TRepository('documentos_turma');
            $limit = 25;
            // creates a criteria
            $criteria = new TCriteria;
            ...
            if (TSession::getValue('documentos_turmaList_filter_data_aula')) {
                $criteria->add(TSession::getValue('documentos_turmaList_filter_data_aula')); // add the session filter
            }
            ...
            if (!empty($turma))
            {
                $criteria->add(new TFilter('turma_id','=',$turma->id));
            }
 ...
 *------------------------------------------------------------------------------*/
    public function onEdit ($param)
    {
         ...
         TSession::setValue('turma_militar',$dados);
         TApplication::loadPage('documentos_turmaForm','onEdit', array('key'=>$param['id']));
         //$this->form->setData($data);
    }//Fim Módulo
/*------------------------------------------------------------------------------
    ...
    public function onViewDocumento ($param)
    {
        ...
        try
        {
            if ($id)
            {
                ...
                $doc = new documentos_turma($id);
                //var_dump($doc);
                $doc_id = $doc->arquivos_externos_id;
                $sql = "SELECT DISTINCT encode(contend, 'base64')as contend FROM sisacad.arquivos_externos WHERE id=".$doc_id;
                ...
                $object = new arquivos_externos($doc_id); // instantiates the Active Record
