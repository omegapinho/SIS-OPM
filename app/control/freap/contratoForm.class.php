...
class contratoForm extends TPage
{
    ...
    var       $opm_ext_nome = "BPMRV";
    var       $form_nome    = "contratoList";
    var       $isWeb        = false;
    ...
    public function __construct( $param )
    {
        ...
        //$this->form->setFormTitle('Gestor de Contratos de Serviço');
        //Cria as abas
        ...
        $table_data->addRowSet(array(new TLabel('Data Inicial:'),$data_inicio,new TLabel('Data Final:'), $data_fim,$calc_button));
        $table_data->addRowSet('<center><center>');
 ...
 *        Atualiza calculos
 *------------------------------------------------------------------------------*/
     public static function onChangeAction_calculos($param)
     {
        if (array_key_exists('id_servico',$param))
        {
        ...
        else//Na falta do identificador do serviço
        {
             $ret = '';
             $obj = new StdClass;
             $obj->valor_total = number_format($ret,2,'.','');//Envia o valor totalizado para o formulário
             TForm::sendData('form_contrato', $obj);            
             return ;
        }
