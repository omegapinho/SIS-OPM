 ...
 */
class OPM_ext_Form extends TPage
{
    protected $form; // form
    ...
    function __construct($param)
    {
        ...
        $sigla->setProperty('style','text-align:left');
        ...
        $table_master->addRowSet( array($save_button, $new_button,$back_button), '', '')->class = 'tformaction'; // CSS class
        ...
        $container->add(new TXMLBreadCrumb('menu.xml', 'OPM_ext_ServicoList'));
    ...
    function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $object = new unidades_ext($key);
                $this->form->setData($object);
                
                $items  = grupo_servico::where('id_opm', '=', $key)->load();
    ...
    public static function onSave($param)
    {
        try
        {
            ...
            $id = (int) $param['id'];
            $master = new unidades_ext;
            $master->fromArray( $param);
            $master->store(); // save master object
            
            // delete details
            grupo_servico::where('id_opm', '=', $master->id)->delete();
 ...
 */
    ...
    {
        $param = null;
        TApplication::loadPage('OPM_ext_ServicoList','OnEdit',$param);
    }    
}
