 ...
 */
class SystemUserForm_Mod extends TPage
{
    protected $form; // form
    ...
    var $servico  = 'Edição';                  //Nome da página de serviço.
    ...
    function __construct()
    {
        ...
        $this->form->setFormTitle( "Usuários de Sistema - Edição" );
        ...
        $unit_id             = new TDBCombo('system_unit_id','permission','SystemUnit','id','nome');
        ...
        //Usar critério de busta de, Não sendo Administrador geral, poder atribuir somente grupos da qual o user tenha
        if ($fer->i_adm())
        {
            $criteria = null;
        }
        else
        {
            ...
            $query = explode(",",TSession::getValue('usergroupids'));
            //print_r($query);
            $criteria->add (new TFilter ('id','IN', $query));
        }
        $groups              = new TDBCheckGroup('groups','permission','SystemGroup','id','name','name',$criteria);
        ...
        $groups->setBreakItems(4);
        foreach ($groups->getLabels() as $label)
        {
            $label->setSize(240);
        }
        
        $frame_groups = new TFrame(NULL, null);
        $frame_groups->setLegend(_t('Groups'));
        $frame_groups->style .= ';margin:0px;width:95%;overflow:auto;';
        $frame_groups->setIsWrapped(true);
        ...
        $this->form->addAction( _t('Back to the listing'), new TAction(array('SystemUserList_Mod','onReload')), 'fa:table blue');
